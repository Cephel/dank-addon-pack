...
function LVBM.Announce(Warning, localWarning)
	if Warning == LVBM.LastWarning then
		if (GetTime() - LVBM.LastWarningTime) < 1 then
			return;
		end
	end
	LVBM.LastWarning = Warning;
	LVBM.LastWarningTime = GetTime();
	local addon = "";	
...
function LVBM.GetBossHP(name)
	...
	if name == LVBM_FOURHORSEMEN_THANE and LVBM.AddOns.FourHorsemen and LVBM.AddOns.FourHorsemen.InCombat then
		return "Mograine "..LVBM.AddOns.FourHorsemen[LVBM_FOURHORSEMEN_MOGRAINE].."%%, Thane Korth'azz "..LVBM.AddOns.FourHorsemen[LVBM_FOURHORSEMEN_THANE].."%%, Blaumeux "..LVBM.AddOns.FourHorsemen[LVBM_FOURHORSEMEN_LADY].."%%, Zeliek "..LVBM.AddOns.FourHorsemen[LVBM_FOURHORSEMEN_ZELIEK].."%%";
	end
	...
	if not hitpoints then
		...
		elseif name == LVBM_GOTH_NAME and LVBM.AddOns.Gothik then
			hitpoints = string.format(LVBM_WAVE, LVBM.AddOns.Gothik.Wave);
		elseif name == LVBM_NTP_NAME and LVBM.AddOns.Noth then
...
--Misc Functions--
------------------
function LVBM.Capitalize(text, xall)
	text = tostring(text);
...
function LVBM.SetHooks()
	...
	function LVBM.Hooks.newPlaySound(snd)
		if (snd == "RaidWarning") and LVBM.Options.Gui then
