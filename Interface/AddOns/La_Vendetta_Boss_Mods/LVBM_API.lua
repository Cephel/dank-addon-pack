...
function LVBM.OnUpdate(elapsed)
	...
	for index, value in pairs(LVBM.AddOns) do --execute the OnUpdate functions of the addons
		if value.Options.Enabled and type(value.OnUpdate) == "function" then
			if value.Instance == GetRealZoneText() or value.Instance == LVBM_OTHER or (not (GetLocale() == "deDE" or GetLocale() == "enGB" or GetLocale() == "enUS")) then		
...
function LVBM.StartStatusBarTimer(timer, name, noBroadcast, syncedBy, startedBy, repeatingTimer, repetitions, class, colorR, colorG, colorB, colorA)
	...
	if getglobal("LVBM_StatusBarTimer"..barId) then
		...
		if LVBM.StatusBarData[name].frame.specialColor and not specialColor then --we need to reset the color if we restart a timer with the same name...but without a specific color
...
function LVBM.EndStatusBarTimer(name, noBroadcast, syncedBy, endRepeating)
	...
	if syncedBy and LVBM.StatusBarData[name].isRepeating and (not endRepeating) and ((not LVBM.SyncInfo.Clients[syncedBy]) or (LVBM.SyncInfo.Clients[syncedBy] and tonumber(LVBM.SyncInfo.Clients[syncedBy]) < 1.60)) then
		return;
	end
...
function LVBM.InterceptWhisper(msg, player)
	if not LVBM.InCombat or not LVBM.HideWhispers or LVBM.Raid[player] or not LVBM.Options.AutoRespond or string.sub(msg, 1, 20) == "<Vendetta Boss Mods>" then
		return "SHOW";
	end
...
function LVBM.OnSyncMessage(msg, player, noSpamProt)
	...
	if not LVBM.Options.AllowSyncFromOldVersions and not (string.find(msg, "VER") or string.find(msg, "ENDALL")) then
		if tonumber(LVBM.SyncInfo.Clients[player]) < tonumber(LVBM.Version) then
