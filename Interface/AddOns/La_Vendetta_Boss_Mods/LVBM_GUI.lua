...
LVBMGUI_TAB_NAXX	= "Naxxramas";
LVBMGUI_TAB_AQ40	= "Ahn'Qiraj";
LVBMGUI_TAB_BWL		= "BlackWingLair";
LVBMGUI_TAB_MC		= "Molten Core";
LVBMGUI_TAB_20PL	= "20 Player";
LVBMGUI_TAB_OTHER	= "Other";
...
--		UIDropDownMenu_AddButton{ notCheckable = 1 }				-- Space --> sucks =[
		...
		LVBMDebugMsg("[Debug][Funktionsaufruf]: LVBMBossModFrameDropDown_Initialize() - UIDROPDOWNMENU_MENU_LEVEL ~= 1");
	end
...
function LVBM_Pull_Announce(msg, announce)
	if( msg == nil and type(announce) == "number" ) then
		LVBM.Announce(string.format("*** Pull in %d sec ***", announce));
	elseif( msg == "pull" ) then
		LVBM.Announce("*** Incoming Pull ***");
	else
		LVBM.UnSchedule("LVBM_Pull_Announce");

		local time = tonumber(msg);
		if( time == nil or time > 30 ) then time = 10;	end

		for i=1, (time-1), 1 do
			LVBM.Schedule(time-i, "LVBM_Pull_Announce", nil, i);
		end
		LVBM.Schedule(time, "LVBM_Pull_Announce", "pull");
...
function LVBMBossModFrame_OnEvent(event)
	if( event == "PLAYER_LOGIN" ) then
		...
		LVBM_Gui_AddTab(LVBMGUI_TAB_NAXX, "Naxx");
		LVBM_Gui_AddTab(LVBMGUI_TAB_AQ40, "AQ40");
		LVBM_Gui_AddTab(LVBMGUI_TAB_BWL, "BWL");
		LVBM_Gui_AddTab(LVBMGUI_TAB_MC, "MC");
		LVBM_Gui_AddTab(LVBMGUI_TAB_20PL, "ZG/Aq20");
		LVBM_Gui_AddTab(LVBMGUI_TAB_OTHER, "Other");
...
-- Debug Output for Developement Help
function LVBMDebugMsg(msg)
	if ( LVBMGUI_DEBUG ) then
