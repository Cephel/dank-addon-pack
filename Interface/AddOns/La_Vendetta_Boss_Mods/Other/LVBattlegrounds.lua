...
-- modivied by Nitram to v1.2 with auto turn-In for alterac valley
-- modivied by Nitram to v1.4 with Arathi Basin Time to End Bar, bugfixes etc.
-- modivied by Nitram to v1.5 with WSG Flag Info Frame and a lot of Bugfixes.
-- modivied by Nitram to v1.6 with only Local Bars, Option to change "bar" text, Colored Bars
...
LVBM.AddOns.Battlegrounds = { 
	...
	["WSGInfoFrameCreate"] = function()
		LVBMWsgInfoFrame = LVBMGui:CreateInfoFrame("Warsong Flag Captures", "Shows the Flag Carrier");
		if( not LVBMWsgInfoFrame ) then 
			LVBM.AddMsg("Can't get Frame from CreateInfoFrame()");
			return false; 
		end

		LVBMWsgInfoFrameAllianceFlag 	= LVBMWsgInfoFrame:CreateStatusBar(0, 1, 1, LVBM_BGMOD_LANG.WSG_ALLYFLAG.."Base");
		...
		LVBMWsgInfoFrameHordeFlag 	= LVBMWsgInfoFrame:CreateStatusBar(0, 1, 1, LVBM_BGMOD_LANG.WSG_HORDEFLAG.."Base");
		...
		if (event == "CHAT_MSG_BG_SYSTEM_ALLIANCE" or event == "CHAT_MSG_BG_SYSTEM_HORDE") and LVBM.AddOns.Battlegrounds.Options.ShowWsgFrame then
			if( LVBMWsgInfoFrame ) then
				if( string.find(arg1, LVBM_BGMOD_LANG.WSG_FLAG_PICKUP) ) then
					local _, _, mSide, mNick =  string.find(arg1, LVBM_BGMOD_LANG.WSG_FLAG_PICKUP);
