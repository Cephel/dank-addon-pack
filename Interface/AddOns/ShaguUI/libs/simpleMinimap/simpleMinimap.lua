...
function simpleMinimap:OnInitialize()
	...
	self.defaults = {
		...
		buttonPos = { bgs=302, meet=189, mail=30, time=1, track=38, zoomin=209, zoomout=235 },
		show = { bgs=true, location=false, locationText=false, mail=true, meet=true, time=false, track=true, zoom=true }
	}
	self.options = {
		type="group",
		args={
...
function simpleMinimap:UpdateScreen()
	...
	elseif(MinimapCluster.smm_SetAlpha) then
		...
		MinimapCluster:SetPoint("TOPRIGHT", UIParent, -1, 6)
